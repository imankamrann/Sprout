...
export const QuestPanelSnackMix: React.FC<QuestPanelSnackMixProps> = ({
  ...
  const handleTwist = (choice: "fuel" | "treat") => {
    ...
    if (choice === "fuel") {
      setReputationDelta((prev) => prev + 1);
  ...
  const handleScam = (choice: "pay" | "ask") => {
    ...
    onPlaySound("click");
    if (choice === "pay") {
      setReputationDelta((prev) => prev - 2);
  ...
  const finishQuest = () => {
    ...
    onResult({
      ...
      xpGained,
      reputationChange: reputationDelta + 2,
      avoidedScam,
      plannedWell,
    });
    ...
    return (
      <div className="quest-card quest-card-result">
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card">
            <div className="stat-value">{reputationDelta >= 0 ? "👍" : "👎"} {reputationDelta >= 0 ? "+" : ""}{reputationDelta + 2}</div>
            <div className="stat-label">{questCopy.result.stats.reputation}</div>
          </div>
        </div>
